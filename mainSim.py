...
if __name__ == '__main__':
    ...
    boolDrawDemandGenGraph = True##################
    boolDrawGenMixGraph = True
    boolDrawCurYearRenGen = True#######
            ...
            for j in range(len(elecGenCompanies)):
                if(elecGenCompanies[j].name == curCompName):
                  ...
                  #      elecGenCompanies[j].addGeneration(tempName, tempTypeID, tempRen, tempCapKW, tempStartYear, tempEndYear, tempAge, coalCapSub, False, coalCapMarketBool)
                        elecGenCompanies[j].addGeneration(tempName, tempTypeID, tempRen, tempCapKW, tempStartYear, tempEndYear, tempAge, yCostPerKWh, True, False, tempbus, OneYearHeadroom[tempbus-1])
                    else:
                        elecGenCompanies[j].addGeneration(tempName, tempTypeID, tempRen, tempCapKW, tempStartYear, tempEndYear, tempAge, 0.0, False, False, tempbus, OneYearHeadroom[tempbus-1])
...
#distcap= difference+come online soon
    ...
    temprand=choice([1,2,7,8,9,10,11,12,13,15,16,19,20,26,27,28,29])
  ...
  #          elecGenCompanies[i].addBatterySize(batteryCapPerCompany)
    ...
    for y in range(maxYears): # Loop through years
        ...
        if(BASEYEAR+y == 2019):
            ...
            for k in range(len(elecGenCompanies)):
                if(elecGenCompanies[k].name == cName):
                    temprand=choice([2,7,10,12,13,14,15,16,17,18,19,20,22,23,24,26,28,30])
                    #elecGenCompanies[k].addGeneration('BECCS', 9, 0, 500000.0, 2025, 2044, 0, 0.0, False, False, temprand, OneYearHeadroom[temprand-1])
        ...
        if(BASEYEAR+y == 2035):
            ...
            for k in range(len(elecGenCompanies)):
                if(elecGenCompanies[k].name in cHydrogenName):
                    temprand=random.randint(1,30)
                    #elecGenCompanies[k].addGeneration('Hydrogen', 11, 0, 75000.0, 2035, 2044, 0, 0.0, False, False, temprand, OneYearHeadroom[temprand-1])
        ...
        #        heatAndGas[c].graph()
            ...
            '''
            sumRes = np.sum(totalCustDemand)
            sumNonRes = np.sum(nonResidentialDemand)
            sumTotGen = np.sum(totalGenProfile)
            print('sumRes    ',sumRes)
            ...
            if(boolDrawCurYearRenGen):
                ...
                Utils.graph(rgGraphData,rgXlab, rgYlab, tempTitle, rgSubtitles, RESULTS_FILE_PATH)
            # --------------------------------------------------------------
